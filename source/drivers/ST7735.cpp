...
void ST7735::sendBytes(const uint8_t *src, unsigned num) {
  ...
  if (double16) {
    ...
    while (num--) {
      uint32_t v = work->expPalette[*src++]; //
      *dst++ = v;
      *dst++ = v >> 8;
      *dst++ = v >> 16;
    }
...
void ST7735::sendWords(uint8_t *src, unsigned numBytes) {
  ...
  // work->srcPtr = (uint8_t *)src;  // unsure why this is needed
  startTransfer((uint8_t *)dst - work->dataBuf);
}
...
void ST7735::startTransfer(unsigned size) {
  io.startSend(work->dataBuf, size, (PVoidCallback)&ST7735::sendColorsStep,
               this);
}
